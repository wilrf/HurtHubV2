...
(async function() {
  ...
  const styles = {
    header: 'background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); color: white; font-size: 14px; padding: 8px; font-weight: bold;',
    subheader: 'background: #4a5568; color: white; font-size: 12px; padding: 6px; font-weight: bold;',
  ...
  const DATABASE = {
    companies: [
      { name: "Lowe's", revenue: 96250000000, employees: 300000, industry: "Retail" },
      { name: "Bank of America", revenue: 94950000000, employees: 213000, industry: "Financial Services" },
      { name: "Honeywell", revenue: 34392000000, employees: 113000, industry: "Technology" },
      { name: "Duke Energy", revenue: 25097000000, employees: 28000, industry: "Utilities" },
      { name: "Sonic Automotive", revenue: 11898000000, employees: 10000, industry: "Automotive" }
    ],
    stats: {
      total_companies: 5,
  ...
  const TESTS = [
    {
      ...
      validate: (response) => {
        ...
        if (!hasRevenue) {
          errors.push({ code: 'E202', msg: 'Wrong revenue (expected $34.392B)' });
        ...
        if (!hasEmployees) {
          errors.push({ code: 'E203', msg: 'Wrong employee count (expected 113,000)' });
    ...
    },
    {
      ...
      validate: (response) => {
        ...
        if (lowePos > -1 && boaPos > -1 && lowePos > boaPos) {
          errors.push({ code: 'E304', msg: 'Wrong order: Bank of America before Lowe\'s' });
        }
    ...
    },
    {
      ...
      question: 'How many total companies are in the database and what is their combined revenue?',
      validate: (response) => {
        const errors = [];
        const passes = [];
        const details = {};
        ...
        if (response.match(/\b5\b|five/i)) {
          ...
          errors.push({ code: 'E302', msg: 'Wrong company count (expected 5)' });
        ...
        if (!foundRevenue) {
          errors.push({ code: 'E302', msg: 'Wrong total revenue (expected $262.6B)' });
        }
  ...
  // Main test execution
  console.clear();
  console.log('%c🧪 AI DATABASE CONNECTION TEST - BROWSER CONSOLE', styles.header);
  ...
  for (const test of TESTS) {
    ...
    console.log('%cQuestion: "%c' + test.question + '%c"', styles.dim, styles.info, styles.dim);
    
    const apiResponse = await callAPI(test.question);
    results.responseTimes.push(apiResponse.responseTime || 0);
    
    if (!apiResponse.success) {
      console.log(`%c✗ API Error [${apiResponse.error}]: ${ERROR_CODES[apiResponse.error]}`, styles.error);
    ...
    console.log(`%c✓ Response received in ${apiResponse.responseTime}ms`, styles.pass);
  ...
  if (results.errors.length > 0) {
    ...
    Object.entries(errorFreq).forEach(([code, count]) => {
      console.log(`  %c[${code}]%c ${ERROR_CODES[code]} (${count}x)`, 
        styles.code, styles.dim);
    });
  }
  ...
  if (successRate >= 90) {
    console.log('%c✅ EXCELLENT: AI is properly connected to database!', styles.pass);
  } else if (successRate >= 70) {
    console.log('%c✅ GOOD: AI has database access with minor issues', styles.pass);
  ...
  console.table(DATABASE.companies.map(c => ({
    Company: c.name,
  ...
  console.log('%cSave as bookmark: javascript:(function(){...script...})();', styles.dim);
