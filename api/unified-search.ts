...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log('Unified Search Request:', { query, searchType, filters, limit });
    ...
    switch (searchType) {
      case 'semantic':
        results = await performSemanticSearch(query, openai, supabase, limit, filters);
      ...
      default:
        ...
        const [semanticResults, keywordResults] = await Promise.all([
          performSemanticSearch(query, openai, supabase, semanticLimit, filters).catch(err => {
    ...
    return res.status(200).json({
      ...
      results: results.map(result => ({
        ...result,
        relevanceScore: result.similarity || calculateKeywordRelevance(query, result),
      })),
...
async function performKeywordSearch(
  ...
  try {
    ...
    dbQuery = dbQuery.order('revenue', { ascending: false, nullsFirst: false }).limit(limit);
...
function applyFilters(results: any[], filters: any): any[] {
  return results.filter(item => {
    if (filters.minRevenue && (item.revenue || 0) < filters.minRevenue) return false;
    if (filters.maxRevenue && (item.revenue || 0) > filters.maxRevenue) return false;
    if (filters.minEmployees && (item.employees_count || 0) < filters.minEmployees) return false;
    if (filters.maxEmployees && (item.employees_count || 0) > filters.maxEmployees) return false;
...
function calculateKeywordRelevance(query: string, result: any): number {
  ...
  const text = `${result.name} ${result.description || ''} ${result.industry || ''}`.toLowerCase();
...
async function enhanceResultsWithAI(query: string, results: any[], openai: OpenAI): Promise<any[]> {
  try {
    ...
    return results.map((result, index) => ({
      ...result,
      aiContext: enhancement.split('\n')[index] || 'Relevant match found based on search criteria.',
      enhanced: true
    }));
...
async function calculateSearchAnalytics(results: any[]) {
  ...
  const totalEmployees = results.reduce((sum, r) => sum + (r.employees_count || 0), 0);
