  ...
  try {
    ...
    for (const table of tables) {
      try {
        ...
        results.dataCounts[table.name as keyof typeof results.dataCounts] =
          count || 0;

        if (error) {
          results.healthy = false;
          results.recommendations.push(
            `Table '${table.table}' is not accessible: ${error.message}`,
          );
        }
      } catch (error: any) {
        ...
        results.healthy = false;
        results.recommendations.push(
          `Failed to check table '${table.table}': ${error.message}`,
        );
      }
    }
    ...
    if (results.dataCounts.companies === 0) {
      results.recommendations.push(
        "No companies found. Consider seeding the database with business data.",
      );
    }

    if (results.dataCounts.ai_conversations === 0) {
      results.recommendations.push(
        "No AI conversations found. This is normal for a new system.",
      );
    }

    if (results.database.latency > 1000) {
      results.recommendations.push(
        `Database latency is high (${results.database.latency}ms). Consider optimizing queries or database location.`,
      );
    }

    // Check if AI tables exist but are empty
    if (
      results.tables.ai_conversations &&
      results.dataCounts.ai_conversations === 0
    ) {
      results.recommendations.push(
        "AI conversation tables exist but are empty. The AI will build context over time.",
      );
    }
  } catch (error: any) {
    results.healthy = false;
