...
class Environment {
  ...
  private loadConfig(): EnvConfig {
    return {
      ...
      ENABLE_REAL_TIME: this.parseBoolean(import.meta.env.VITE_ENABLE_REAL_TIME, true),
      ENABLE_ANALYTICS: this.parseBoolean(import.meta.env.VITE_ENABLE_ANALYTICS, false),
      ENABLE_EXPORT: this.parseBoolean(import.meta.env.VITE_ENABLE_EXPORT, true),
      ...
      SHOW_DEV_TOOLS: this.parseBoolean(import.meta.env.VITE_SHOW_DEV_TOOLS, false),
    }
  }
  ...
  private validateConfig(): void {
    ...
    if (this.config.ENABLE_ANALYTICS && !this.config.GOOGLE_ANALYTICS_ID) {
      console.warn('⚠️ Analytics are enabled but GOOGLE_ANALYTICS_ID is not set')
    }
    ...
    if (this.isDevelopment()) {
      ...
      console.log(`⚡ Real-time: ${this.config.ENABLE_REAL_TIME ? 'enabled' : 'disabled'}`)
      console.log(`📊 Mock API: ${this.config.MOCK_API ? 'enabled' : 'disabled'}`)
    }
  }
  ...
  hasAnalytics(): boolean {
    return Boolean(this.config.GOOGLE_ANALYTICS_ID) && this.config.ENABLE_ANALYTICS
  }
  ...
  updateFeatureFlag<K extends keyof Pick<EnvConfig, 'ENABLE_AI_FEATURES' | 'ENABLE_REAL_TIME' | 'ENABLE_ANALYTICS' | 'ENABLE_EXPORT'>>(
    flag: K,
    value: boolean
  ): void {
    if (this.isDevelopment()) {
