...
export function BusinessProfile() {
  ...
  const loadBusinessData = async (businessId: string) => {
    setIsLoading(true)
    try {
      const businessData = await businessDataService.getBusinessById(businessId)
    ...
    return (
      <div className="p-6">
        <div className="text-center py-20">
          ...
          <Button onClick={() => navigate('/')} variant={isDarkMode ? 'glass' : 'default'}>
    ...
    <div className="p-6">
      ...
      <div className="flex space-x-1 mb-6">
        {[
          ...
          <Button
            key={tab.id}
            variant={selectedTab === tab.id ? (isDarkMode ? 'glass' : 'default') : 'ghost'}
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm">{formatNumber(business.squareFeet)} sq ft</p>
                      </div>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card variant={isDarkMode ? 'glass' : 'elevated'}>
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="text-center p-4 border rounded-lg">
                      <p className="text-2xl font-bold">{formatPercentage(business.grossMargin)}</p>
                      <p className="text-xs text-muted-foreground">Gross Margin</p>
                    </div>
                    <div className="text-center p-4 border rounded-lg">
                      <p className="text-2xl font-bold">{formatPercentage(business.netMargin)}</p>
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between p-4 border rounded-lg">
                      <span className="text-lg font-semibold">{formatCurrency(business.revenuePerEmployee)}</span>
                      <Badge variant="secondary">
                        Industry Average: {formatCurrency(business.revenuePerEmployee * 0.85)}
                      </Badge>
                    </div>
                  </div>
                      ...
                      {business.operatingCosts && Object.entries(business.operatingCosts).map(([category, amount]) => (
                        <div key={category} className="flex items-center justify-between">
                          ...
                          <div className="flex items-center">
                            <div className={`w-32 h-2 rounded-full mr-3 ${
              ...
              <Card variant={isDarkMode ? 'glass' : 'elevated'}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Rent per Sq Ft</p>
                      <p className="text-sm">${business.rentPerSqFt}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Annual Rent</p>
                      <p className="text-sm">{formatCurrency(business.annualRent)}</p>
                    </div>
                    <div>
                ...
                <CardContent>
                  <div className="flex items-center gap-6 mb-6">
                    <div className="text-center">
                      <p className="text-3xl font-bold">{business.rating?.toFixed(1) || 'N/A'}</p>
                      ...
                      <p className="text-sm text-muted-foreground">{business.reviewCount} reviews</p>
                    </div>
                  </div>
          ...
          <Card variant={isDarkMode ? 'glass' : 'elevated'}>
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Employee Category</span>
                <span className="text-sm font-medium">{business.employeeSizeCategory}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Founded</span>
                <span className="text-sm font-medium">{business.yearEstablished}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Business Age</span>
                <span className="text-sm font-medium">{business.businessAge} years</span>
              </div>
              <div className="flex justify-between">
