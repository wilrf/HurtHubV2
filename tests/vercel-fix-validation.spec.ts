...
test.describe("Static Asset Serving Fix Validation", () => {
  test("manifest.json should return valid JSON, not HTML", async ({
    request,
    page,
  }) => {
  ...
  test("static assets should load correctly", async ({ request, page }) => {
    ...
    const successfulAssets = assetsRequests.filter(
      (req) => req.status() === 200,
    );
  ...
  test("improvedDemoData.json should return JSON if it exists", async ({
    request,
  }) => {
  ...
  test("should not have Vite import analysis errors", async ({ page }) => {
    ...
    const viteErrors = consoleErrors.filter(
      (error) =>
...
test.describe("React Router Functionality", () => {
  ...
  test("non-existent routes should serve SPA for React Router", async ({
    page,
  }) => {
...
test.describe("API Endpoints", () => {
  test("API routes should not be affected", async ({ request }) => {
    ...
    if (response.status() === 200) {
      ...
      console.log(
        `ℹ️ API endpoint returned ${response.status()} - may need environment setup`,
      );
    }
  });
});
