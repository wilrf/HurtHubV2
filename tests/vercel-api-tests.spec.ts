...
test.describe("Database-First API Implementation", () => {
  test.describe("/api/businesses endpoint", () => {
    test("should fetch businesses from database", async ({ request }) => {
      ...
      console.log(
        `✅ Fetched ${data.businesses.length} businesses from database`,
      );
    });
    ...
    test("should handle business search with filters", async ({ request }) => {
      ...
      console.log(
        `✅ Search returned ${data.businesses.length} filtered results`,
      );
    });
    ...
    test("should provide analytics data", async ({ request }) => {
      ...
      console.log(
        `✅ Analytics: ${data.analytics.totalCompanies} companies, ${data.analytics.topIndustries.length} industries`,
      );
    });
    ...
    test("should handle pagination correctly", async ({ request }) => {
      ...
      console.log(
        `✅ Pagination working: Page 1 (${data1.businesses.length}), Page 2 (${data2.businesses.length})`,
      );
    });
  });
  ...
  test.describe("/api/ai-search endpoint", () => {
    test("should perform AI-powered search", async ({ request }) => {
      ...
      console.log(
        `✅ AI Search: Found ${data.results.length} results with intent analysis`,
      );
    ...
    test("should handle semantic search with embeddings", async ({
      ...
      if (data.results.length > 0) {
        const hasSemanticResults = data.results.some(
          (r: any) => r.searchType === "semantic",
        );
        console.log(
          `✅ Semantic search ${hasSemanticResults ? "ACTIVE" : "FALLBACK TO KEYWORD"}`,
        );
      }
    });
  });
  ...
  test.describe("/api/generate-embeddings endpoint", () => {
    test("should check embedding generation capability", async ({
      request,
    }) => {
      ...
      console.log(
        `✅ Embeddings: ${data.message}, processed: ${data.processed}`,
      );
    });
  });

  test.describe("Error Handling (No Fallbacks)", () => {
    test("should fail explicitly with missing parameters", async ({
      request,
    }) => {
      const response = await request.post(`/api/ai-search`, {
...
test.describe("Frontend Database Integration", () => {
  test("should load business data in frontend", async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="business-count"], .business-card, .loading',
      { timeout: 10000 },
    );

    // Check if businesses are loaded from API
    const businessElements = await page
      .locator('.business-card, [data-testid="business-item"]')
      .count();
    ...
    if (businessElements > 0) {
      console.log(
        `✅ Frontend loaded ${businessElements} businesses from database`,
      );
    ...
    await page.screenshot({
      path: "tests/screenshots/business-intelligence-loaded.png",
    });
  });
...
test.describe("AI Chat Database Integration", () => {
  test("should use database context in AI chat", async ({ page }) => {
    ...
    const chatButton = page
      .locator(
        '[data-testid="ai-chat"], button:has-text("AI"), button:has-text("Chat")',
      )
      .first();
      ...
      if ((await chatInput.count()) > 0) {
        ...
        if (lastMessage) {
          ...
          const hasCompanyData =
            messageText?.includes("Bank of America") ||
            messageText?.includes("revenue") ||
            messageText?.includes("Charlotte");

          console.log(
            `✅ AI Chat ${hasCompanyData ? "USING DATABASE CONTEXT" : "response received"}`,
          );
          await page.screenshot({
            path: "tests/screenshots/ai-chat-response.png",
          });
        }
      }
    } else {
