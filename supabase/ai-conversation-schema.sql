...
CREATE TABLE IF NOT EXISTS ai_conversations (
  ...
  INDEX idx_session_id ON ai_conversations(session_id),
  INDEX idx_user_id ON ai_conversations(user_id),
  INDEX idx_created_at ON ai_conversations(created_at DESC)
);
...
CREATE TABLE IF NOT EXISTS ai_session_summaries (
  ...
  INDEX idx_summary_session ON ai_session_summaries(session_id),
  INDEX idx_summary_user ON ai_session_summaries(user_id)
);
...
CREATE TABLE IF NOT EXISTS ai_analytics (
  ...
  INDEX idx_analytics_session ON ai_analytics(session_id),
  INDEX idx_analytics_user ON ai_analytics(user_id),
  INDEX idx_analytics_type ON ai_analytics(analysis_type)
);
...
CREATE TABLE IF NOT EXISTS ai_feedback (
  ...
  INDEX idx_feedback_session ON ai_feedback(session_id),
  INDEX idx_feedback_rating ON ai_feedback(rating)
);
...
CREATE TABLE IF NOT EXISTS ai_context_cache (
  ...
  INDEX idx_cache_key ON ai_context_cache(cache_key),
  INDEX idx_cache_expires ON ai_context_cache(expires_at)
);
