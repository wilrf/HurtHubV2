...
const runDiagnostics = async () => {
  ...
  console.log(COLORS.info('━'.repeat(50)));
  console.log(COLORS.dim(`Environment: ${process.env.NODE_ENV || 'development'}`));
  ...
  if (!hasAllEnvVars) {
    console.log('\n' + COLORS.warning('⚠️  Missing environment variables. Some features may not work.'));
  }

  // Test API endpoints (skip for local dev since Vercel functions don't run locally)
  if (isLocal) {
    console.log('\n' + COLORS.dim('📡 API health checks skipped (Vercel functions not available locally)'));
    console.log(COLORS.dim('   Run `vercel dev` to test API endpoints locally'));
  } else {
    ...
    if (diagData.checks?.openai?.hasKey && !diagData.checks?.openai?.lengthValid) {
      console.log(COLORS.warning(`    ⚠️  Key length: ${diagData.checks.openai.keyLength} (expected: ${diagData.checks.openai.expectedLength})`));
    }
    
    // Supabase Status
    const supabaseStatus = diagData.checks?.supabase?.connection?.success;
    const supabaseIcon = supabaseStatus ? COLORS.success('✓') : COLORS.error('✗');
    const supabaseMsg = supabaseStatus ? 
      COLORS.success(`Connected (${diagData.checks.supabase.connection.companyCount} companies)`) : 
      COLORS.error(diagData.checks?.supabase?.connection?.error || 'Failed');
    console.log(`  ${supabaseIcon} Supabase: ${supabaseMsg}`);
    
    // Overall Status
    console.log('\n' + COLORS.info('📊 Overall Status:'));
    const statusColor = diagData.status === 'healthy' ? COLORS.success : 
                       diagData.status === 'degraded' ? COLORS.warning : COLORS.error;
    console.log(`  System Health: ${statusColor(diagData.status.toUpperCase())}`);
