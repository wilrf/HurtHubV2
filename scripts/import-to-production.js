...
if (!supabaseKey) {
  ...
  console.error(
    '   SUPABASE_SERVICE_ROLE_KEY="your-key-here" node scripts/import-to-production.js',
  );
  process.exit(1);
}
...
async function clearExistingData() {
  console.log(
    "🗑️  Clearing existing test data (keeping only top 5 companies)...",
  );
...
async function importData() {
  try {
    ...
    console.log(
      `📝 Currently ${initialCount || 0} companies in production database`,
    );
    ...
    for (let i = 0; i < companies.length; i += batchSize) {
      ...
      if (newCompanies.length === 0) {
        console.log(
          `⏭️  Batch ${batchNumber} skipped: all companies already exist`,
        );
      ...
      if (error) {
        ...
        console.log(
          `✅ Batch ${batchNumber}: imported ${data.length}, skipped ${batch.length - newCompanies.length}`,
        );
      }
...
importData()
  .then(() => {
    ...
    console.log(
      "🎯 Your AI assistant now has access to all 299 companies in production!",
    );
    process.exit(0);
  })
  .catch((error) => {
