...
async function testAIChat() {
  ...
  try {
    ...
    const messages = await page.locator('[class*="message"], div[role="log"], .prose').allTextContents();
    const lastMessage = messages.length > 0 ? messages[messages.length - 1] : 'No response found';
    console.log('🤖 AI Response:', lastMessage.substring(0, Math.min(200, lastMessage.length)) + (lastMessage.length > 200 ? '...' : ''));
    
    // Test 2: Ask about restaurants
    console.log('\n📝 Test 2: Asking about Charlotte restaurants...');
    await chatInput.fill('List some Charlotte food service businesses like Albertine, Bos on Noda, or Kitchen + Kocktails');
    await chatInput.press('Enter');
    
    await page.waitForTimeout(5000);
    
    const messages2 = await page.locator('[class*="message"], div[role="log"], .prose').allTextContents();
    const lastMessage2 = messages2.length > 0 ? messages2[messages2.length - 1] : 'No response found';
    console.log('🤖 AI Response:', lastMessage2.substring(0, Math.min(200, lastMessage2.length)) + (lastMessage2.length > 200 ? '...' : ''));
    ...
    const messages3 = await page.locator('[class*="message"], div[role="log"], .prose').allTextContents();
    const lastMessage3 = messages3.length > 0 ? messages3[messages3.length - 1] : 'No response found';
    console.log('🤖 AI Response:', lastMessage3.substring(0, Math.min(200, lastMessage3.length)) + (lastMessage3.length > 200 ? '...' : ''));
    ...
    const allResponses = messages.concat(messages2, messages3).join(' ').toLowerCase();
    
    console.log('\n📊 Analysis Results:');
    if (allResponses.includes('299') || allResponses.includes('two hundred ninety')) {
    ...
    if (allResponses.includes('albertine') || allResponses.includes('kitchen') || allResponses.includes('food')) {
      console.log('✅ AI can access restaurant data!');
    } else {
      console.log('⚠️ AI might not be seeing restaurant data');
    }
    
    if (allResponses.includes('honeywell') || allResponses.includes('technology')) {
