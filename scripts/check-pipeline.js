...
function getGitHubToken() {
  ...
  console.log(
    `${colors.yellow}ℹ No GitHub token found. Some features may be limited.${colors.reset}`,
  );
  console.log(
    "To set a token, run: git config --global github.token YOUR_TOKEN",
  );
  return null;
}
...
async function checkPipeline() {
  ...
  console.log(
    `${colors.gray}Repository: ${REPO_OWNER}/${REPO_NAME}${colors.reset}\n`,
  );
  ...
  try {
    ...
    for (const run of runs.workflow_runs) {
      ...
      console.log(
        `${status.emoji} ${status.color}${status.text}${colors.reset} - ${colors.bold}${run.name}${colors.reset}`,
      );
      console.log(`   Branch: ${colors.blue}${run.head_branch}${colors.reset}`);
      console.log(
        `   Commit: ${colors.gray}${run.head_sha.substring(0, 7)}${colors.reset} - ${run.head_commit?.message?.split("\n")[0] || "No message"}`,
      );
      console.log(
        `   Started: ${formatTime(run.created_at)} | Duration: ${formatDuration(run.created_at, run.updated_at)}`,
      );
    ...
    console.log(
      `${colors.gray}Or run: npx playwright install && npm run test:e2e:ui${colors.reset}\n`,
    );
  } catch (error) {
    console.error(
      `${colors.red}Error fetching pipeline status:${colors.reset}`,
      error.message,
    );
