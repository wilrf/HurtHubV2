...
async function testCoffeeShops() {
  ...
  const { data: coffeeShops, error: coffeeError } = await supabase
    ...
    .or('name.ilike.%coffee%,name.ilike.%dunkin%,name.ilike.%starbucks%,description.ilike.%coffee%')
    .eq('status', 'active')
    .limit(10);
  ...
  if (coffeeError) {
    ...
    console.log(`Found ${coffeeShops?.length || 0} coffee-related businesses:\n`);
  ...
  if (!indError && industries) {
    ...
    console.log(`\nSample industries (${uniqueIndustries.length} unique):`, uniqueIndustries.slice(0, 10).join(', '));
  }
  
  // Search for Huntersville/Davidson businesses specifically
  console.log('\n--- Searching for Huntersville/Davidson businesses ---');
  const { data: localBiz, error: localError } = await supabase
    .from('companies')
    .select('name, city, address')
    .or('city.ilike.%huntersville%,city.ilike.%davidson%,address.ilike.%huntersville%,address.ilike.%davidson%')
    .eq('status', 'active')
    .limit(10);
  
  if (!localError && localBiz) {
    console.log(`Found ${localBiz?.length || 0} businesses in Huntersville/Davidson:`);
    localBiz?.forEach(biz => {
      console.log(`- ${biz.name}: ${biz.address || 'N/A'}, ${biz.city || 'N/A'}`);
    });
  }
}
