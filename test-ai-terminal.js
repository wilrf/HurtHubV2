...
const TEST_SUITE = [
  {
    ...
    validate: (response) => {
      ...
      if (!lower.includes('honeywell')) {
        errors.push({ code: 'E204', detail: 'Honeywell not mentioned in response' });
      } else {
      ...
      const hasRevenue = revenueFormats.some(format => response.includes(format));
      ...
      const fakeCompanies = ['microsoft', 'google', 'amazon', 'wells fargo', 'jpmorgan'];
  ...
  },
  {
    ...
    validate: (response) => {
      ...
      const companies = ["lowe's", "bank of america", "honeywell", "duke energy", "sonic automotive"];
      let lastIndex = -1;
      ...
      if (lower.includes('database') || lower.includes('our data') || lower.includes('records')) {
  ...
  },
  {
    ...
    question: 'Calculate the revenue per employee for Duke Energy and Sonic Automotive',
    validate: (response) => {
      const errors = [];
      const warnings = [];
      const passes = [];
      ...
      if (!hasSonicCalc) {
        errors.push({
          code: 'E301',
          detail: 'Sonic Automotive revenue/employee wrong. Expected: $1,189,800'
        });
      } else {
        passes.push('Sonic Automotive calculation correct: $1,189,800/employee');
      }
  ...
  },
  {
    ...
    question: 'How many companies are in the Technology industry and what is their total revenue?',
    validate: (response) => {
      const errors = [];
      const warnings = [];
      const passes = [];
      ...
      if (lower.includes('honeywell')) {
        ...
        errors.push({ code: 'E204', detail: 'Failed to identify Honeywell as Technology company' });
      }
      ...
      if (lower.includes('1') || lower.includes('one')) {
        ...
        errors.push({ code: 'E302', detail: 'Wrong count of technology companies (should be 1)' });
      }
      
      // Should NOT mention other companies as tech
      if (lower.includes('bank of america') && lower.includes('tech')) {
        errors.push({ code: 'E201', detail: 'Incorrectly classified Bank of America as Technology' });
      }
  ...
  },
  {
    ...
    question: 'How many total companies are in the database and what is their combined revenue and total employees?',
    validate: (response) => {
      const errors = [];
      const warnings = [];
      const passes = [];
      ...
      if (response.includes('5') || response.includes('five')) {
        ...
        errors.push({ code: 'E302', detail: 'Wrong company count (should be 5)' });
      }
      ...
      const companies = ["lowe's", "bank of america", "honeywell", "duke energy", "sonic"];
      const mentionedCompanies = companies.filter(c => response.toLowerCase().includes(c));
      
      if (mentionedCompanies.length === 5) {
        passes.push('All 5 companies mentioned by name');
      } else if (mentionedCompanies.length > 0) {
        warnings.push(`Only ${mentionedCompanies.length}/5 companies mentioned`);
      }
...
async function runTests() {
  ...
  console.log(`${colors.bold}${colors.blue}║           AI DATABASE CONNECTION TEST - TERMINAL              ║${colors.reset}`);
  console.log(`${colors.bold}${colors.blue}╚════════════════════════════════════════════════════════════════╝${colors.reset}`);
  console.log(`${colors.dim}Endpoint: ${API_URL}/api/ai-chat-simple${colors.reset}`);
  ...
  for (const test of TEST_SUITE) {
    ...
    const apiResult = await testAPI(`${API_URL}/api/ai-chat-simple`, test.question);
    
    if (!apiResult.success) {
      console.log(`${colors.red}✗ API Error [${apiResult.error}]: ${ERROR_CODES[apiResult.error]}${colors.reset}`);
      ...
      console.log(`${colors.green}✓ Response received (${apiResult.responseTime}ms)${colors.reset}`);
      ...
      if (validation.errors.length > 0) {
        ...
        validation.errors.forEach(err => {
          console.log(`${colors.red}    [${err.code}] ${ERROR_CODES[err.code]}${colors.reset}`);
  ...
  // Calculate averages
  results.avgResponseTime = Math.round(results.avgResponseTime / TEST_SUITE.length);
  ...
  const total = results.totalErrors + results.totalWarnings + results.totalPasses;
  const successRate = total > 0 ? Math.round((results.totalPasses / total) * 100) : 0;
  
  console.log(`\n${colors.bold}Overall Results:${colors.reset}`);
  console.log(`  ${colors.green}Passes:   ${results.totalPasses}${colors.reset}`);
  console.log(`  ${colors.yellow}Warnings: ${results.totalWarnings}${colors.reset}`);
  ...
  if (results.errorCodes.size > 0) {
    ...
    Array.from(results.errorCodes).sort().forEach(code => {
      console.log(`  ${colors.red}[${code}]${colors.reset} ${ERROR_CODES[code]}`);
    });
  }
  ...
  if (successRate >= 90) {
    console.log(`${colors.green}${colors.bold}✓ EXCELLENT: AI is properly connected to database!${colors.reset}`);
  } else if (successRate >= 70) {
    console.log(`${colors.green}${colors.bold}✓ GOOD: AI has database access but needs minor improvements${colors.reset}`);
  } else if (successRate >= 50) {
    console.log(`${colors.yellow}${colors.bold}⚠ PARTIAL: AI has limited database connectivity${colors.reset}`);
  } else if (successRate >= 20) {
    console.log(`${colors.red}${colors.bold}✗ POOR: AI is mostly using generic responses${colors.reset}`);
  } else {
    console.log(`${colors.red}${colors.bold}✗ FAILED: AI is not connected to database at all${colors.reset}`);
  }
  ...
  console.log(`${colors.dim}Database truth: 5 companies, $262.6B total revenue, 664K employees${colors.reset}\n`);
}
