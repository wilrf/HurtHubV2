...
export function getOpenAIClient() {
  if (!client) {
    ...
    if (!apiKey.startsWith("sk-")) {
      throw new Error(
        `Invalid OpenAI key format: ${apiKey.substring(0, 10)}...`,
      );
    }
...
export default async function handler(req, res) {
  ...
  if (report.checks.openai.hasKey && !report.checks.openai.lengthValid) {
    report.recommendations.push(
      "OpenAI key length is incorrect - regenerate key or check for extra characters",
    );
  }

  if (report.checks.openaiConnection?.status === 401) {
    report.recommendations.push(
      "OpenAI key is invalid - verify key in OpenAI dashboard",
    );
  }
...
- [ ] Add OPENAI*API_KEY to Vercel (no VITE* prefix)
- [ ] Deploy application
...
export function validateConfig() {
  ...
  if (!process.env.OPENAI_API_KEY) {
    ...
    errors.push(
      `OPENAI_API_KEY has wrong length: ${process.env.OPENAI_API_KEY.length}`,
    );
  }
...
7. **Never expose keys client-side** - No VITE\_ prefix
